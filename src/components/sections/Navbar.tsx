    ...
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled ? 'backdrop-blur-md bg-white/90 border-b border-slate-100 py-4 shadow-sm' : 'py-6 bg-white border-b border-transparent'
        }`}
      >
        <div className="max-w-[1200px] mx-auto px-6 md:px-10 flex items-center justify-between">
          <a href="#" className="flex items-center gap-2 group outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 rounded">
              ...
              {NAV_LINKS.map(link => {
                ...
                return (
                  <a 
                    key={link.label}
                    href={link.href} 
                    className={`relative text-sm font-medium transition-colors py-1 ${isActive ? 'text-slate-900 font-bold' : 'text-slate-500 hover:text-indigo-600'}`}
                  >
                    {link.label}
                    <span 
