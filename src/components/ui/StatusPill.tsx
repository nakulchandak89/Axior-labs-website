...
export function StatusPill({ label, variant }: StatusPillProps) {
  ...
  return (
    <div className={`inline-flex items-center font-sans text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${colors[variant as keyof typeof colors]}`}>
      {isLive ? (
        <span className={`w-2 h-2 rounded-full ${dotColors.live} animate-pulse mr-2`}></span>
      ) : (
        <span className={`w-2 h-2 rounded-full mr-2 ${dotColors[variant as keyof typeof dotColors]}`}></span>
      )}
      {label}
    </div>
  );
